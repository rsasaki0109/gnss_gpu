#!/usr/bin/env python3
"""WP12e probe ablation: dense anchors + calibrated cert."""
...
# Full-length-calibrated cert (WP12e): tighter recovery + anchor proximity.
CERT = [
...
STAGES = [
    ("float_dense_anchor", []),
    ("cert_dense_hold", ["--lambda-ar"] + CERT),
]
    ...
    for name, extra in STAGES:
        ...
        if force and pos.exists():
            ...
            if telem.exists():
                telem.unlink()
        if not pos.exists():
            cmd = COMMON + ["--export-pos", str(pos), "--telemetry-csv", str(telem)] + extra
        ...
        print(
            f"{name}: AllRMS={score.get('all_rms_m'):.3f} FixRMS={score.get('fix_rms_m')} "
            f"<50cm_full%={score.get('lt50cm_full_pct')}",
            flush=True,
        )
    (OUT / "ablation_4000_summary.json").write_text(
        json.dumps({"probe_epochs": 4000, "rows": rows}, indent=2) + "\n", encoding="utf-8"
    )
    return 0
