"""WP23a: score every ablation-grid .pos output and merge with health-stat CSVs.

Not a permanent pipeline module -- a one-off aggregation script for the
WP23A report table, mirroring results/wp22b/score_grid.py's pattern. Reads
the .pos files under results/wp23a/pos/<config>_<arm>/ and the per-run
health-stat CSVs under experiments/results/wp23a_<config>_<arm>_runs.csv,
...
POS_ROOT = REPO / "results" / "wp23a" / "pos"
CSV_ROOT = REPO / "experiments" / "results"
OUT_CSV = REPO / "results" / "wp23a" / "csv" / "wp23a_grid_scored.csv"

CONFIGS = {
    "baseline": "RBPF-velKF+DD+gate",
    "cpmupf": "RBPF-velKF+DD+CP-MUPF+gate",
}
ARMS = ["off", "preint"]
RUNS = ["run1", "run2", "run3"]
...
HEALTH_COLS = [
    ...
    "resample_rate",
    "epoch_tempering",
    "epoch_tempering_mean_alpha",
    "epoch_tempering_mean_post_ratio",
    "dd_epochs_attempted",
    "dd_epochs_applied",
    "dd_pairs_total",
    "mupf_pr_epochs_applied",
    "mupf_pr_pairs_total",
    "mupf_pr_resample_rate",
    "mupf_pr_mean_alpha",
    "mupf_pr_mean_post_ess_ratio",
    "mupf_cp_epochs_applied",
    "mupf_cp_pairs_total",
    "mupf_cp_stage_calls",
    "mupf_cp_resample_rate",
    "mupf_cp_mean_alpha",
    "mupf_cp_mean_post_ess_ratio",
    "mupf_cp_slips_flagged_total",
    "mupf_cp_slip_epochs",
...
def load_health_row(config: str, arm: str, run: str) -> dict:
    csv_path = CSV_ROOT / f"wp23a_{config}_{arm}_runs.csv"
...
def main() -> None:
    ...
    out_rows = []
    for config, method_label in CONFIGS.items():
        for arm in ARMS:
            for run in RUNS:
                pos_path = POS_ROOT / f"{config}_{arm}" / f"tokyo_{run}_{method_label}.pos"
