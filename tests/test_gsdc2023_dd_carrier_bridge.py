...
def test_bridge_carrier_wavelength_uses_signal_band() -> None:
    assert carrier_wavelength_m(1, "GPS_L1_CA") == GPS_L1_WAVELENGTH
    assert carrier_wavelength_m(1, "GPS_L5_Q") == GPS_L5_WAVELENGTH


def test_bridge_rover_measurements_for_epoch_converts_adr_meters_to_cycles() -> None:
    batch = SimpleNamespace(
        adr=np.array([[GPS_L1_WAVELENGTH * 1000.0, GPS_L5_WAVELENGTH * 2000.0]], dtype=np.float64),
        adr_state=np.array([[1, 1]], dtype=np.int32),
        sat_ecef=np.array(
            [
                [
        ...
        slot_keys=((1, 3, "GPS_L1_CA"), (6, 11, "GAL_E5_Q")),
    )
...
def test_bridge_snap_tow_to_base_epoch_uses_nearest_within_tolerance() -> None:
...
def test_bridge_base_obs_template_prefers_existing_course_file(tmp_path) -> None:
