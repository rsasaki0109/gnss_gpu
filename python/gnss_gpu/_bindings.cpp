...
PYBIND11_MODULE(_gnss_gpu, m) {
  ...
  // --- Positioning ---
  m.def("wls_position", [](py::array_t<double, py::array::c_style | py::array::forcecast> sat_ecef,
                            py::array_t<double, py::array::c_style | py::array::forcecast> pseudoranges,
                            py::array_t<double, py::array::c_style | py::array::forcecast> weights,
                            int max_iter, double tol) {
    auto bs = sat_ecef.request(), bp = pseudoranges.request(), bw = weights.request();
    if (bp.ndim != 1)
      throw std::runtime_error("wls_position: pseudoranges must have shape (n_sat,)");
    if (bw.ndim != 1)
      throw std::runtime_error("wls_position: weights must have shape (n_sat,)");
    int n_sat = bp.size;
    if (n_sat < 4)
      throw std::runtime_error("wls_position requires at least 4 satellites");
    if (bw.size != n_sat)
      throw std::runtime_error("wls_position: weights length must match pseudoranges length");
    if (!((bs.ndim == 1 && bs.size == n_sat * 3) ||
          (bs.ndim == 2 && bs.shape[0] == n_sat && bs.shape[1] == 3)))
      throw std::runtime_error("wls_position: sat_ecef must have shape (n_sat, 3) or flat length n_sat*3");
    if (max_iter < 1)
      throw std::runtime_error("wls_position: max_iter must be >= 1");
    if (tol <= 0.0)
      throw std::runtime_error("wls_position: tol must be positive");
  ...
  m.def("wls_batch", [](py::array_t<double, py::array::c_style | py::array::forcecast> sat_ecef,
                         py::array_t<double, py::array::c_style | py::array::forcecast> pseudoranges,
                         py::array_t<double, py::array::c_style | py::array::forcecast> weights,
                         int max_iter, double tol) {
    auto bs = sat_ecef.request(), bp = pseudoranges.request(), bw = weights.request();
    if (bs.ndim != 3 || bs.shape[2] != 3)
      throw std::runtime_error("wls_batch: sat_ecef must have shape (n_epoch, n_sat, 3)");
    if (bp.ndim != 2)
      throw std::runtime_error("wls_batch: pseudoranges must have shape (n_epoch, n_sat)");
    if (bw.ndim != 2)
      throw std::runtime_error("wls_batch: weights must have shape (n_epoch, n_sat)");
    int n_epoch = bs.shape[0];
    int n_sat = bs.shape[1];
    if (n_epoch < 1)
      throw std::runtime_error("wls_batch: n_epoch must be >= 1");
    if (n_sat < 4)
      throw std::runtime_error("wls_batch requires at least 4 satellites");
    if (bp.shape[0] != n_epoch || bp.shape[1] != n_sat)
      throw std::runtime_error("wls_batch: pseudoranges shape must match sat_ecef epochs/satellites");
    if (bw.shape[0] != n_epoch || bw.shape[1] != n_sat)
      throw std::runtime_error("wls_batch: weights shape must match sat_ecef epochs/satellites");
    if (max_iter < 1)
      throw std::runtime_error("wls_batch: max_iter must be >= 1");
    if (tol <= 0.0)
