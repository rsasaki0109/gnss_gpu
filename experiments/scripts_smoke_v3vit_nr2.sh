#!/usr/bin/env bash
# Smoke: v3 ranker + Viterbi α=0.5 + k=99 on n/r2.
...
V3VIT_CSV=experiments/results/selector_ranker_predictions_v3_viterbi_a05.csv
...
PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
  ...
  --n-particles 2000 --pos-dir "/tmp/ppc_smoke_v3vit_k99_${CITY}_${RUN}" \
  --results-prefix "ppc_smoke_v3vit_k99_${CITY}_${RUN}_full" \
  >"/tmp/smoke_v3vit_k99_${CITY}_${RUN}.log" 2>&1
PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v3vit_k99_${CITY}_${RUN}_full_runs.csv")
