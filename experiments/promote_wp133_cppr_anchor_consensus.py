#!/usr/bin/env python3
"""Promote a holdout-validated WP133 CP/PR-anchored constant offset."""
...
from select_wp133_cppr_anchor_consensus import select
...
def main() -> int:
    ...
    if not target["accepted"]:
        raise RuntimeError("target CP/PR-anchor selector does not pass")
    ...
    for label in ("holdout_a", "holdout_b"):
        ...
        holdouts[label] = _verify(
            recomputed, getattr(args, f"{label}_selection"), hashes
        )
    ...
    result = {
        "schema": "wp133_cppr_anchor_promotion_v1",
        "production_input_truth": False,
        "truth_usage": "none",
        "production_promoted": True,
        "reason": "unique_cppr_anchor_cross_basis_mode",
