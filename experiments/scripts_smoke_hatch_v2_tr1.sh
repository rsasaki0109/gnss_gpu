#!/usr/bin/env bash
# Phase 32 v2 smoke: atmosphere-corrected Hatch SPP candidate added to Phase 29 pool.
# tokyo/run1 v1+k3 baseline = 90.84%.
set -uo pipefail
...
G_HATCH=experiments/results/libgnss_hatch_alite_v2

OLD_LABELS=$(cat /tmp/${CITY}_${RUN}_phase11fa_labels.txt)
OLD_DIRS=$(cat /tmp/${CITY}_${RUN}_phase11fa_dirs.txt)
NEW_DIRS="${OLD_DIRS},$FGO_V2_DIR,$FGO_V14_DIR,$FGO_V17_DIR,$G_DEFAULT,$G_Z,$G_R,$G_LP,$G_LH,$G_R4,$G_COMBO,$G_C4,$G_LL,$G_ZR,$G_OA,$G_LA,$G_HS,$G_HS45,$G_HS30,$G_HE,$G_IR,$G_MB,$G_W5,$G_HATCH"
NEW_LABELS="${OLD_LABELS},xd_fgo_v2_gap,xd_fgo_v14_snr38,xd_fgo_v17_el25,xd_gici_def,xd_gici_z,xd_gici_r,xd_gici_lp,xd_gici_lh,xd_gici_r4,xd_gici_combo,xd_gici_c4,xd_gici_lprlph,xd_gici_zr,xd_gici_oa,xd_gici_la,xd_gici_hs,xd_gici_hs45,xd_gici_hs30,xd_gici_he,xd_gici_ir,xd_gici_mb,xd_gici_w5,xd_hatch"

echo "=== Phase 32 v2 hatch (atmo+MultiGNSS) tokyo/run1 v1+k3 (baseline 90.84%) ==="
PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
  ...
  --rtkdiag-candidate-ranker-score-path "$V1_CSV" \
  ...
  --n-particles 2000 --pos-dir "/tmp/ppc_smoke_hatch_v2_${CITY}_${RUN}" \
  --results-prefix "ppc_smoke_hatch_v2_${CITY}_${RUN}_full" \
  >"/tmp/smoke_hatch_v2_${CITY}_${RUN}.log" 2>&1
PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_hatch_v2_${CITY}_${RUN}_full_runs.csv")
echo "FIN hatch_v2 ${CITY}/${RUN}: ${PPC}% (baseline 90.8415%)"

echo ""
echo "=== xd_hatch pick rate ==="
grep -c "xd_hatch" /tmp/smoke_hatch_v2_${CITY}_${RUN}.log || echo "0 (never picked)"
