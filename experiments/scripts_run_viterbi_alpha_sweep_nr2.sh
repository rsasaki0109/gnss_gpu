#!/usr/bin/env bash
# Viterbi α sweep on n/r2. α=0.5 gave +0.48pp. Test 0.2/0.3/1.0.
set -uo pipefail
...
run_variant() {
  local ALPHA=$1
  local TAG=$2
  local CSV="experiments/results/selector_ranker_predictions_viterbi_a${TAG}.csv"
  if [ ! -f "$CSV" ]; then
    echo "MISSING $CSV - skipping"
    return
  fi
  echo "=== Viterbi α=${ALPHA} (${TAG}) ${CITY}/${RUN} ==="
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_vit_${TAG}_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_vit_${TAG}_${CITY}_${RUN}_full" \
    >"/tmp/vit_${TAG}_${CITY}_${RUN}.log" 2>&1
  local PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_vit_${TAG}_${CITY}_${RUN}_full_runs.csv")
  echo "FIN α=${ALPHA} ${CITY}/${RUN}: ${PPC}% (baseline 62.65%, a05=63.13%)"
}
