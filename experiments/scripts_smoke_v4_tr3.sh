...
# Phase 26 v1+Viterbi = 63.13, Phase 28 v3+k3 = 63.01. Combine?
set -uo pipefail
...
PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
  ...
  --n-particles 2000 --pos-dir "/tmp/ppc_smoke_v4_k3_${CITY}_${RUN}" \
  --results-prefix "ppc_smoke_v4_k3_${CITY}_${RUN}_full" \
  >"/tmp/smoke_v4_k3_${CITY}_${RUN}.log" 2>&1
PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v4_k3_${CITY}_${RUN}_full_runs.csv")
echo "FIN v3+vit+k3 ${CITY}/${RUN}: ${PPC}%"
echo ""
echo "=== n/r2 comparison ==="
echo "  v1+k3 (Phase 20):       62.6501"
echo "  v1+viterbi (Phase 26):  63.1306"
echo "  v3+k3 (Phase 27):       62.6351"
echo "  v3+k3 (Phase 28):      63.0105"
echo "  v3+viterbi+k3 (smoke): ${PPC}%"
