#!/usr/bin/env bash
# Phase 24: Viterbi α=0.5 smoothed ranker predictions on all 6 runs.
...
VITERBI_CSV=experiments/results/selector_ranker_predictions_viterbi_a05.csv

# Skip n/r2 since smoke already done
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase24_viterbi_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_phase24_viterbi_${CITY}_${RUN}_full" \
    >"/tmp/phase24_viterbi_${CITY}_${RUN}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase24_viterbi_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 24 Viterbi a05 OFFICIAL aggregate ==="
TOTAL=0.0
N=0
# n/r2 from smoke result
NR2=$(awk -F, 'NR==2 {printf "%.4f", $11}' experiments/results/ppc_ctrbpf_fgo_viterbi_a05_nagoya_run2_full_runs.csv)
echo "  nagoya/run2: ${NR2}% (smoke)"
TOTAL=$(awk "BEGIN {printf \"%.4f\", $TOTAL + $NR2}")
N=1
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase24_viterbi_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
