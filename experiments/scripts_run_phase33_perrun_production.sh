#!/usr/bin/env bash
# Phase 33 production deploy: per-run conditional v5_nlos ranker.
# Configuration:
#   v1+k3:        tokyo/run1, tokyo/run3, nagoya/run3   (Phase 29 retain)
#   v3+k3:        tokyo/run2                            (Phase 29 retain, v5 -0.014pp regression)
#   v3+k99:       nagoya/run1                           (Phase 29 retain, v5 -0.57pp regression)
#   v5_nlos+k99:  nagoya/run2                           (NEW +1.07pp dominant gain)
# Expected OFFICIAL: 85.9403% = +0.18pp vs Phase 29 / +0.34pp vs TURING.
set -uo pipefail
...
declare -A CONFIG
CONFIG[tokyo_run1_csv]="$V1_CSV"; CONFIG[tokyo_run1_k]="3";  CONFIG[tokyo_run1_lbl]="v1"
CONFIG[tokyo_run2_csv]="$V3_CSV"; CONFIG[tokyo_run2_k]="3";  CONFIG[tokyo_run2_lbl]="v3"
CONFIG[tokyo_run3_csv]="$V1_CSV"; CONFIG[tokyo_run3_k]="3";  CONFIG[tokyo_run3_lbl]="v1"
CONFIG[nagoya_run1_csv]="$V3_CSV"; CONFIG[nagoya_run1_k]="99"; CONFIG[nagoya_run1_lbl]="v3"
CONFIG[nagoya_run2_csv]="$V5_CSV"; CONFIG[nagoya_run2_k]="99"; CONFIG[nagoya_run2_lbl]="v5_nlos"
CONFIG[nagoya_run3_csv]="$V1_CSV"; CONFIG[nagoya_run3_k]="3";  CONFIG[nagoya_run3_lbl]="v1"
...
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  echo "=== Phase 33 PROD ${CITY}/${RUN} (ranker=${LBL}, k=${K}) ==="
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase33_prod_${KEY}" \
    --results-prefix "ppc_ctrbpf_fgo_phase33_prod_${KEY}_full" \
    >"/tmp/phase33_prod_${KEY}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase33_prod_${KEY}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 33 production deploy (per-run conditional v5_nlos) ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase33_prod_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
...
echo "  OFFICIAL: ${AVG}% (n=${N}) [Phase 29 85.7623% / Expected 85.94%]"
