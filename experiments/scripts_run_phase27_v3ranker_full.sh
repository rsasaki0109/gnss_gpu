#!/usr/bin/env bash
# Phase 27: v3 ranker (cluster-topology + temporal features) full 6-run sweep.
# Baseline: Phase 20 v1 OFFICIAL 85.47%, Phase 26 conditional 85.55%.
# Recoverable ceiling on n/r2: +3.115pp (1930 epochs / 18.69% n/r2 path).
set -uo pipefail
...
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase27_v3_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_phase27_v3_${CITY}_${RUN}_full" \
    >"/tmp/phase27_v3_${CITY}_${RUN}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase27_v3_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 27 v3 ranker OFFICIAL aggregate ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase27_v3_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
...
echo "  OFFICIAL: ${AVG}% (n=${N}) [Phase 20 v1 85.47% / Phase 26 cond 85.55%]"
