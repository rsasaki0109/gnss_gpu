#!/usr/bin/env bash
# Phase 28: v3 ranker + rms_prefilter_k=99 (no prefilter) full 6-run sweep.
# Hypothesis confirmed on n/r2 smoke: v3+k99 = 63.01% vs Phase 20 v1+k3 62.65% (+0.36pp).
# v3-v1 @ k99 delta = +0.50pp ≈ LORO sim +0.46pp (realization 109%).
# Projected OFFICIAL: ~86.10% = +0.63pp vs Phase 20 85.47%, TURING 85.6% 超え見込み.
set -uo pipefail
...
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --rtkdiag-candidate-ranker-score-path "$V3_CSV" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase28_v3k99_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_phase28_v3k99_${CITY}_${RUN}_full" \
    >"/tmp/phase28_v3k99_${CITY}_${RUN}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase28_v3k99_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 28 v3+k99 OFFICIAL aggregate ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase28_v3k99_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
...
echo "  OFFICIAL: ${AVG}% (n=${N}) [Phase 20 85.47% / Phase 26 85.55% / TURING 85.6%]"
