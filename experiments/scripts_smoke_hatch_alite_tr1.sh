#!/usr/bin/env bash
# Phase 32 smoke: hatch_alite SPP candidate added to Phase 29 pool, tokyo/run1 v1+k3 config.
# Baseline: Phase 29 t/r1 v1+k3 = 90.84% (production-best).
set -uo pipefail
...
G_HATCH=experiments/results/libgnss_hatch_alite_v1
...
echo "=== Phase 32 hatch_alite smoke (tokyo/run1 v1+k3, baseline 90.84%) ==="
PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
  ...
  --n-particles 2000 --pos-dir "/tmp/ppc_smoke_hatch_${CITY}_${RUN}" \
  --results-prefix "ppc_smoke_hatch_${CITY}_${RUN}_full" \
  >"/tmp/smoke_hatch_${CITY}_${RUN}.log" 2>&1
PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_hatch_${CITY}_${RUN}_full_runs.csv")
echo "FIN hatch ${CITY}/${RUN}: ${PPC}%"
echo "  baseline Phase 29 v1+k3: 90.8415%"
