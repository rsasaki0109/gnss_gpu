#!/usr/bin/env bash
# Phase 25: Viterbi α=0.5 smoothed ranker predictions on all 6 runs.
...
VITERBI_CSV=experiments/results/selector_ranker_predictions_viterbi_a02.csv

# All 6 runs with α=0.2
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase25_viterbi_a02_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_phase25_viterbi_a02_${CITY}_${RUN}_full" \
    >"/tmp/phase25_viterbi_a02_${CITY}_${RUN}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase25_viterbi_a02_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 25 Viterbi a02 OFFICIAL aggregate ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase25_viterbi_a02_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
