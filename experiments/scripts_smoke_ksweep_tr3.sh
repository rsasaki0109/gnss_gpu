#!/usr/bin/env bash
# k sweep on t/r3 (max LORO gain +2.02pp but PF negative at k=3 and k=99).
# Test k=10 and k=20 middle values to see if sweet spot exists.
set -uo pipefail
...
run_k() {
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_ksweep_v3k${K}_${CITY}_${RUN}" \
    --results-prefix "ppc_smoke_v3_k${K}_${CITY}_${RUN}_full" \
    >"/tmp/ksweep_v3k${K}_${CITY}_${RUN}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v3_k${K}_${CITY}_${RUN}_full_runs.csv")
  echo "FIN v3 k=${K} ${CITY}/${RUN}: ${PPC}%"
}
...
echo ""
echo "=== t/r3 k sweep on v3 ranker ==="
echo "  v1+k3 baseline (Phase 20): 88.9492%"
echo "  v3+k3  (Phase 27):         88.4795%"
echo "  v3+k99 (Phase 28):         88.3029%"
K10=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v3_k10_${CITY}_${RUN}_full_runs.csv")
K20=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v3_k20_${CITY}_${RUN}_full_runs.csv")
echo "  v3+k10:                    ${K10}%"
echo "  v3+k20:                    ${K20}%"
