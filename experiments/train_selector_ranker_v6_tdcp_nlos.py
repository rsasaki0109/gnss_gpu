#!/usr/bin/env python3
"""Train selector ranker v6 with TDCP-style multi-step deltas + PLATEAU NLOS features.

Pipeline: v3 base (28 features) + 5 TDCP features (v6 extractor) + 6 NLOS features
(augment script) = 39 features total. v5 was 28+6=34. v6 adds:
- delta_pos_2step_m, delta_pos_3step_m: longer-lag position smoothness
- delta_pos_vertical_m, delta_pos_horizontal_m: axis-split delta (NLOS often
  shows vertical glitches)
- delta_pos_accel_m: 2nd-derivative of trajectory (smoothness)

Input: experiments/results/selector_training_features_v6_nlos_tdcp.csv
"""
...
FEATURE_COLS = [
    ...
    # TDCP features (v6 NEW)
    "delta_pos_2step_m", "delta_pos_3step_m",
    "delta_pos_vertical_m", "delta_pos_horizontal_m",
    "delta_pos_accel_m",
]
...
def fit_one(train_df, valid_df, label_categories):
    ...
    return lgb.train(
        LGB_PARAMS, train_set, num_boost_round=200,
...
def main():
    feat_path = REPO / "experiments/results/selector_training_features_v6_nlos_tdcp.csv"
    ...
    out_pred = REPO / "experiments/results/selector_ranker_predictions_v6_tdcp_nlos.csv"
    pred_all.to_csv(out_pred, index=False)
    print(f"\nSaved: {out_pred}", flush=True)

    result = evaluate_official(pred_all)
    print(f"\n=== Ranker v6_tdcp_nlos LORO OFFICIAL: {result['official'] * 100:.4f}% ===", flush=True)
    ...
    out_model = REPO / "experiments/results/selector_ranker_model_v6_tdcp_nlos.txt"
