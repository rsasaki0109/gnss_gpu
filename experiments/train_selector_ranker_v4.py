...
def main():
    feat_path = REPO / "experiments/results/selector_training_features_v4.csv"
    ...
    out_pred = REPO / "experiments/results/selector_ranker_predictions_v4.csv"
    pred_all.to_csv(out_pred, index=False)
    print(f"\nSaved: {out_pred}", flush=True)

    result = evaluate_official(pred_all, label_col="p_pass")
    print(f"\n=== Ranker v4 LORO OFFICIAL: {result['official'] * 100:.2f}% ===", flush=True)
    ...
    out_model = REPO / "experiments/results/selector_ranker_model_v4.txt"
