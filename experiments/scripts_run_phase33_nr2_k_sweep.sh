#!/usr/bin/env bash
# Phase 33 Stage 34a: nagoya/run2 rms_prefilter_k sweep with v5_nlos ranker.
# Production currently uses v5_nlos+k=99. Test k=3, k=5, k=10 to see if a
# stricter prefilter beats k=99 (no prefilter).
set -uo pipefail
...
for K in 3 5 10; do
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --rtkdiag-candidate-ranker-score-path "$V5_CSV" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase33_nr2_v5_k${K}" \
    --results-prefix "ppc_ctrbpf_fgo_phase33_nr2_v5_k${K}_full" \
    >"/tmp/phase33_nr2_v5_k${K}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase33_nr2_v5_k${K}_full_runs.csv")
  echo "FIN nagoya/run2 v5_k${K}: ${PPC}%"
done

echo ""
echo "=== Phase 33 nagoya/run2 v5_nlos k sweep ==="
echo "  k=99 (production): 64.0783% (baseline)"
for K in 3 5 10; do
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase33_nr2_v5_k${K}_full_runs.csv" 2>/dev/null)
  echo "  k=${K}: ${P}%"
done
