#!/usr/bin/env bash
# Phase 43 production deploy: Phase 33 v5_nlos plus n/r2 runtime GICI override.
# Configuration:
#   v1+k3:                 tokyo/run1, tokyo/run3, nagoya/run3
#   v3+k3:                 tokyo/run2
#   v3+k99:                nagoya/run1
#   v5_nlos+k99+override:  nagoya/run2
# Expected OFFICIAL projection: ~85.9983% (Phase 33 85.9403% + n/r2 +0.3483/6).
set -uo pipefail
...
CONFIG[nagoya_run2_csv]="$V5_CSV"; CONFIG[nagoya_run2_k]="99"; CONFIG[nagoya_run2_mode]="ranker_gici_cluster_override"
...
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  echo "=== Phase 43 PROD ${CITY}/${RUN} (mode=${MODE}, k=${K}) ==="
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase43_prod_${KEY}" \
    --results-prefix "ppc_ctrbpf_fgo_phase43_prod_${KEY}_full" \
    >"/tmp/phase43_prod_${KEY}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase43_prod_${KEY}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 43 production deploy ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase43_prod_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
...
echo "  OFFICIAL: ${AVG}% (n=${N}) [Phase 33 85.9403% / projected ~85.9983%]"
