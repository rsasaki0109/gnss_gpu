#!/usr/bin/env bash
# Phase 20: ranker select_mode on all 6 runs
set -uo pipefail
...
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase20_ranker_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_phase20_ranker_${CITY}_${RUN}_full" \
    >"/tmp/phase20_ranker_${CITY}_${RUN}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase20_ranker_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

# Aggregate OFFICIAL = mean of per-run path-weighted
echo ""
echo "=== Phase 20 Ranker OFFICIAL aggregate ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase20_ranker_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
...
if [ "$N" -gt 0 ]; then
  ...
  echo "  OFFICIAL: ${AVG}% (n=${N})"
fi
echo "  baseline Phase 19aw K=3: 83.42%"
echo "  LORO sim: 85.25%"
