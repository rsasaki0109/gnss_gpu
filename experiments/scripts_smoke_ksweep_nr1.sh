#!/usr/bin/env bash
# k sweep on n/r1 (v3+k99 = 83.70 +1.04pp gain).
# Check k=30 and k=50 middle values for further gain.
set -uo pipefail
...
echo ""
echo "=== n/r1 k sweep on v3 ranker ==="
echo "  v1+k3 baseline (Phase 20): 82.6583%"
echo "  v3+k3  (Phase 27):         82.5735%"
echo "  v3+k99 (Phase 28):         83.7003%"
K30=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v3_k30_${CITY}_${RUN}_full_runs.csv")
K50=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_smoke_v3_k50_${CITY}_${RUN}_full_runs.csv")
echo "  v3+k30:                    ${K30}%"
echo "  v3+k50:                    ${K50}%"
