...
def preprocessing_gap_rows() -> list[PreprocessingGapRow]:
    ...
    return [
        ...
        PreprocessingGapRow(
            ...
            python_coverage="optional raw bridge flag",
            python_evidence="validate_fgo_gsdc2023_raw.py --observation-mask",
            status="experimental",
            gap="C/N0, multipath, State bit, range, elevation, pseudorange-Doppler, pseudorange residual, Doppler residual, and ADR-Doppler TDCP consistency masks are connected; carrier artifact and phone_data.mat numerical parity remain partial.",
            next_action="Run audit_gsdc2023_preprocessing_gap.py --scan-trips --bridge-counts on a tree with phone_data.mat and compare count diffs.",
        ),
        PreprocessingGapRow(
            ...
            python_coverage="optional raw bridge flag",
            python_evidence="validate_fgo_gsdc2023_raw.py --dual-frequency",
            status="experimental",
            gap="L1/L5 observations are separate slots and dual-frequency raw bridge/native FGO now use MATLAB's seven signal-clock indices for GPS/Galileo/BeiDou L1/L5. QZSS is mapped to the GPS clock family because MATLAB's sysfreq2sigtype does not assign a QZSS-specific index. Residual parity still needs phone_data.mat validation.",
            next_action="Compare L1/L5 factor counts and residual values against phone_data.mat before enabling by default.",
        ),
        PreprocessingGapRow(
