...
def preprocessing_gap_rows() -> list[PreprocessingGapRow]:
    ...
    return [
        ...
        PreprocessingGapRow(
            ...
            status="experimental",
            gap="L1/L5 observations are separate slots and dual-frequency raw bridge/native FGO now use MATLAB's seven signal-clock indices for GPS/Galileo/BeiDou L1/L5. In the taroz_sn full-parity clock model, QZSS is tagged as Other=7 outside the seven-dimensional clock vector, while non-Taroz bridge modes keep the legacy GPS-family QZSS clock mapping for solver compatibility. Current MATLAB factor-count exports are GPS-only, so multi-GNSS count deltas are scope differences rather than parity failures.",
        ...
        PreprocessingGapRow(
            ...
            python_coverage="optional weak body/ECEF/Taroz body-gravity delta priors plus bias telemetry; native attitude + 6D IMU bias scaffold",
            python_evidence="preintegrate_processed_imu + validate_fgo_gsdc2023_raw.py --imu-prior --imu-frame {body,ecef,taroz_body} --imu-accel-bias-state --factor-dt-max-s; imu_acc/gyro_bias_mean_norm metrics; fgo_gnss_lm_vd optional [attx,atty,attz,bax,bay,baz,bgx,bgy,bgz] state",
            status="experimental",
            gap="Native FGO can consume epoch delta priors, ECEF mode applies yaw/mounting/gravity approximation, raw BiasX/Y/Z are synchronized as telemetry, and long-gap factor gating follows MATLAB time_diff_th. Raw bridge can opt into a minimal attitude plus 6D IMU bias state: --taroz-marupaku preintegration uses the Taroz/GTSAM sample-dt convention equivalent to integrateMeasurement(..., acc.dt(imuIndex)), applies the GTSAM Rot3.RzRyRx mounting angle for taroz_body samples, rotates acceleration plus accel-bias Jacobians by cumulative gyro in sample order, passes an SO(3) Log(deltaR) gyro-bias Jacobian for delta-angle correction, passes direct preintegrated delta p/v gyro-bias Jacobians, stores bias Jacobians in the same sensor-frame ConstantBias convention as GTSAM, and now carries sample-propagated 9D accel/gyro/integration covariance blocks from the same sample-dt recursion; native IMU residuals use preintegration delta_t_s/deltaTij instead of graph epoch dt; accel/gyro bias correct native delta p/v residuals; gyro bias corrects the SO(3) delta-angle residual, with the older delta p/v gyro rotation-term correction kept only as a fallback when direct p/v gyro Jacobians are absent; --taroz-marupaku uses the epoch i+1 bias state inside IMU delta residuals to match Taroz ImuFactor(..., keyB2, ...); delta-angle and default delta p/v factors now use SO(3) Exp/Log rotation instead of rotvec addition or first-order attitude cross terms; taroz_body preintegration matches MATLAB/GTSAM raw and bias-corrected deltaR/P/V at about 1e-6 on the 20-epoch pixel5 smoke, keeps p/v/angle deltas in body frame, and supplies ENU-down gravity converted to ECEF through native imu_gravity for the GTSAM ImuFactor P/V topology: predict state j from pose_i/vel_i/preintegration, then evaluate P/V in R_j.T with predicted-minus-actual sign; bias between factors use Taroz sqrt(numel(IMUindices)) sample-count scaling; dense 9D IMU information matrices are formed by combining the sample-propagated covariance blocks with Taroz AccSigma/GyroSigma/IntegrationSigma. The same-state MATLAB/GTSAM residual CSV join matches at about 1.7e-6 on the 20-epoch pixel5 smoke. The remaining gap is end-to-end optimized trajectory parity and any deeper combined preintegration semantics not represented by the native CUDA approximation.",
            next_action="Extend the same-state GTSAM residual comparison beyond the 20-epoch smoke and then compare optimized native trajectories against fgo_gnss_imu.m.",
        ),
        PreprocessingGapRow(
