#!/usr/bin/env bash
# Phase 23: ranker stickiness sequence smoothing on n/r2.
# Hypothesis: per-epoch independent picks oscillate between candidates; sticking
# to previous label when p_pass>=s*max_p reduces switching cost.
set -uo pipefail
...
run_stickiness() {
  local S=$1
  local TAG=$2
  echo "=== Phase 23 stickiness=${S} ${TAG} ==="
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --rtkdiag-candidate-ranker-stickiness "$S" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase23_${TAG}_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_phase23_${TAG}_${CITY}_${RUN}_full" \
    >"/tmp/phase23_${TAG}_${CITY}_${RUN}.log" 2>&1
  local PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase23_${TAG}_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${TAG} (s=${S}) ${CITY}/${RUN}: ${PPC}% (baseline 62.65%)"
}

run_stickiness 0.95 s095
run_stickiness 0.85 s085
run_stickiness 0.70 s070
