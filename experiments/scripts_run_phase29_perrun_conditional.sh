#!/usr/bin/env bash
# Phase 29: per-run conditional config for production deploy.
# Per-run best config:
#   v1+k3: t/r1, t/r3, n/r3  (Phase 20 baseline preserved)
#   v3+k3: t/r2              (Phase 27 result)
#   v3+k99: n/r1, n/r2       (Phase 28 result)
# Expected OFFICIAL: 85.7623% = +0.29pp vs Phase 20 85.47%, TURING 85.6% +0.16pp 超え.
set -uo pipefail
...
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  ...
  echo "=== Phase 29 ${CITY}/${RUN} (ranker=${RANKER_LABEL}, k=${K}) ==="
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_phase29_${KEY}" \
    --results-prefix "ppc_ctrbpf_fgo_phase29_perrun_${KEY}_full" \
    >"/tmp/phase29_${KEY}.log" 2>&1
  PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase29_perrun_${KEY}_full_runs.csv")
  echo "FIN ${CITY}/${RUN}: ${PPC}%"
done

echo ""
echo "=== Phase 29 per-run conditional OFFICIAL aggregate ==="
TOTAL=0.0
N=0
for CR in tokyo/run1 tokyo/run2 tokyo/run3 nagoya/run1 nagoya/run2 nagoya/run3; do
  CITY=${CR%/*}
  RUN=${CR#*/}
  P=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase29_perrun_${CITY}_${RUN}_full_runs.csv" 2>/dev/null)
  if [ -n "$P" ]; then
...
echo "  OFFICIAL: ${AVG}% (n=${N}) [Phase 20 85.47% / Phase 26 85.55% / TURING 85.6% / Expected 85.76%]"
