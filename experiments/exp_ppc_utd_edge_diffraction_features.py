...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--geoid-correction",
        default="egm96",
        help="PLATEAU height datum correction: 'egm96' (default), 'none', or float metres",
    )
...
def main() -> None:
    ...
    print(
        f"  BVH built in {time.monotonic() - t0:.1f}s ({accelerator.n_triangles} triangles); "
        f"kinds={kinds} geoid={geoid_corr!r}",
        flush=True,
    )
