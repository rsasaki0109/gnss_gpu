#!/usr/bin/env bash
# Viterbi smoothing smoke on n/r2 with alpha=0.5 (already pre-computed predictions).
set -uo pipefail
...
run_variant() {
  ...
  PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
    ...
    --n-particles 2000 --pos-dir "/tmp/ppc_viterbi_${TAG}_${CITY}_${RUN}" \
    --results-prefix "ppc_ctrbpf_fgo_viterbi_${TAG}_${CITY}_${RUN}_full" \
    >"/tmp/viterbi_${TAG}_${CITY}_${RUN}.log" 2>&1
  local PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_viterbi_${TAG}_${CITY}_${RUN}_full_runs.csv")
  echo "FIN ${TAG} ${CITY}/${RUN}: ${PPC}% (baseline 62.65%)"
}

run_variant a05 "experiments/results/selector_ranker_predictions_viterbi_a05.csv"
