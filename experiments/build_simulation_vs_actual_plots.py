...
def render_time_series(city: str, run: str, pos: pd.DataFrame, windows: pd.DataFrame, output_path: Path) -> None:
    fig, (ax_pred, ax_q) = plt.subplots(2, 1, figsize=(11, 5.5), sharex=True, gridspec_kw={"height_ratios": [3, 1.2]})

    ax_pred.step(windows["window_start_s"], windows["corrected_pred_fix_rate_pct"], where="post",
                 color="#e74c3c", linewidth=2.0, label="predicted (§7.16)")
    ax_pred.step(windows["window_start_s"], windows["actual_fix_rate_pct"], where="post",
                 color="#34495e", linewidth=1.5, linestyle="--", label="actual (per-window FIX %)")
    roll = _rolling_fix_fraction(pos["q"], window=75)
    ax_pred.plot(pos["elapsed_s"], roll, color="#2ecc71", linewidth=1.0, alpha=0.8,
                 label="rolling actual FIX fraction (75-epoch)")
...
def render_summary_grid(runs: list[tuple[str, str, pd.DataFrame, pd.DataFrame]], output_path: Path) -> None:
    fig, axes = plt.subplots(2, 3, figsize=(18, 8), sharey=True)
    axes = axes.ravel()
    for ax, (city, run, pos, windows) in zip(axes, runs):
        ax.step(windows["window_start_s"], windows["corrected_pred_fix_rate_pct"], where="post",
                color="#e74c3c", linewidth=1.8, label="predicted")
        ax.step(windows["window_start_s"], windows["actual_fix_rate_pct"], where="post",
                color="#34495e", linewidth=1.3, linestyle="--", label="actual")
        roll = _rolling_fix_fraction(pos["q"], window=75)
        ax.plot(pos["elapsed_s"], roll, color="#2ecc71", linewidth=0.8, alpha=0.7, label="rolling FIX %")
    ...
    fig.suptitle("PPC demo5 FIX-rate — predicted (§7.16) vs actual, all 6 runs", fontsize=13)
...
def main() -> None:
    ...
    cached: list[tuple[str, str, pd.DataFrame, pd.DataFrame]] = []
