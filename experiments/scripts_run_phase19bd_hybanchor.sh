...
PYTHONPATH=python python experiments/exp_ppc_ctrbpf_fgo.py --runs "${CITY}/${RUN}" \
  ...
  --rtkdiag-candidate-select-mode hybrid_anchor \
  ...
  --n-particles 2000 --pos-dir "/tmp/ppc_phase19bd_hyban_${CITY}_${RUN}" \
  --results-prefix "ppc_ctrbpf_fgo_phase19bd_hyban_${CITY}_${RUN}_full" \
  >"/tmp/phase19bd_hyban_${CITY}_${RUN}.log" 2>&1
PPC=$(awk -F, 'NR==2 {printf "%.4f", $11}' "experiments/results/ppc_ctrbpf_fgo_phase19bd_hyban_${CITY}_${RUN}_full_runs.csv")
