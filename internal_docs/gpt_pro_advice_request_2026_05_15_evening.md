...
`gici_open_ws/` に既に clone 済、 全 build deps installed、 PPC2024 dataset 整合 (imu.csv→bin converter 必要)。 `PppEstimator` + `PppImuTcEstimator` (本物の TC FGO) 動かせば候補品質激改善見込。 +1-2pp、 2-3 週。
