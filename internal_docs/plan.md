...
**最終更新**: 2026-05-22 JST (Phase 80: DD anchor gate wire-up `744294e` + build_submission CLI `69acb23` landing、 ebf-x/mi8 empirical chunk_epochs verification 300 rows flip / pos delta median 4.43m 確証、 40-trip chunk_epochs=100 re-run kick)
...
**最近の進捗ハイライト** (新しい順):
- **2026-05-22 Phase 80 production wire-up + ebf-x/mi8 empirical chunk_epochs verification + 40-trip chunk_epochs=100 re-run kick**: Phase 74 で導出した DD anchor coverage gate (`dd_carrier_min_anchor_coverage`、 default 0.6) を inert 状態から production data flow に wire-up (commit `744294e`、 6 files +514/-6、 79 tests pass)。 `build_gsdc2023_bridge_submission.py` 492-line CLI を main に landing (commit `69acb23`)。 ebf-x/mi8 で `--chunk-epochs 200` vs `100` single-trip smoke を実 dataflow で実行、 200 で sources={'baseline': 1924, 'fgo': 0}, 100 で sources={'baseline': 1624, 'fgo': 300}、 **300 rows baseline→fgo flip**、 position delta median **4.43m** / p95 **8.07m** / max **10.45m**。 Phase 77 prediction "ebf-x +300 fgo / +115 rows>5m" と完全一致、 既存 baseline `gsdc2023_bridge_gated_rescue_pixel4_full_20260520/.../ebf-x/mi8/` が chunk_epochs=200 reference と bit-identical 確認。 続いて 40-trip chunk_epochs=100 full re-run kick (`experiments/results/gsdc2023_bridge_chunk100_repro_20260522/`、 PID 254464、 jobs=1、 想定 3-4h)、 既存 chunk_epochs=200 baseline との bridge_positions.csv 差分で Phase 77 src_diff_rows=5091 / rows>5m=1168 quantification を row-level で再現確認予定。 詳細 plan.md §B `45. Phase 80`。
...
45. **Phase 80 production wire-up + ebf-x/mi8 empirical verification + 40-trip re-run (2026-05-21〜22)**: Phase 74 で導出した DD anchor coverage gate を production data flow に wire-up し、 Phase 78 hypothesis を analytical (98.6% match) と empirical (ebf-x/mi8 single-trip) の 2 軸で実 dataflow 確証、 最後に 40-trip 全 trip 再生成を kick。
    ...
    - **40-trip chunk_epochs=100 full re-run kick (2026-05-22)**: 既存 baseline (chunk_epochs=200) との row-level decisive comparison のため、 sample_submission.csv 全 40 trip を `--chunk-epochs 100` で再生成。 出力 `experiments/results/gsdc2023_bridge_chunk100_repro_20260522/`、 PID 254464、 jobs=1 (GPU 排他)、 想定 runtime ~3-4h。 完了後 `experiments/results/gsdc2023_bridge_gated_rescue_pixel4_full_20260520/` との bridge_positions.csv 差分を全 trip で取り、 src_diff_rows 5091 / rows>5m 1168 の Phase 77 quantification と row-level で再現確認できれば chunk_epochs revert の row-level evidence が完成。
    - **次の lever (revert PR scope draft)**: hypothesis 確証後の最小変更は `experiments/build_gsdc2023_bridge_submission.py` の `--chunk-epochs` default を 200 のまま固定 + production submission script の `chunk_epochs=100` 上書きを 200 に revert。 ただし DD-carrier path が 100-epoch granularity に依存する設計箇所があれば壊れる可能性、 Phase 74 anchor coverage gate と組み合わせて DD path だけ 100 維持の per-path 分岐 (revert plan 順位 4) も option。 PR scope は (1) chunk_epochs revert + (2) regression test (40-trip output vs 20260520 baseline の sources bit-identical 確認) + (3) memory 反映、 大きさ ~150 LOC 想定。 private-floor reference 復旧なしでは Kaggle 検証不能のため local row-delta validation で止め、 submit は freeze 維持。
