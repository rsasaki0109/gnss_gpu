...
**現在の HEAD**: `012cd61`  
**gnss_gpu repo の状態**: clean（untracked: `_cmake_build/`, `experiments/diagnose_spp_rtklib_gap.py`）  
**workspace 側の注意**: `ref/gsdc2023/*` と `ref/RTKLIB-demo5/*` は `gnss_gpu` リポジトリの外側にある。workspace ローカルで管理。

**ドキュメント先頭の読み方**

- **セクション F（最優先）**: 2026-04-14 時点の **VD solver + Multi-GNSS + TDCP + Kaggle 提出結果 + MATLAB 同等化ロードマップ**。**いま Claude に引き継ぐなら必ずここから読め。**
- **セクション E（補助）**: 2026-04-12 時点の **外れ trip 診断 + gated source fallback**。
- **セクション D（旧）**: 2026-04-11 時点の GSDC2023 raw-bridge。
- **セクション C（旧）**: 2026-04-08 時点の GPU FGO + RTKLIB SPP 整合。
- **セクション A（旧）**: 2026-04-07 時点の RTKLIB demo5 SPP 整合。C に包含済み。
