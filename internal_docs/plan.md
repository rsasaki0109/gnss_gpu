...
- **2026-05-21 GSDC2023 Kaggle pivot 現況**: TaroZ `dataset_2023.zip` を外部参照 `../ref/gsdc2023/taroz/` として統合し、 `fgo_dd_carrier` guarded full submission を生成・提出。 Kaggle `public=4.048 / private=4.860` で、前回 bridge `3.993/4.821` より悪く、 mtv700/private-floor `~3.686/~4.710` からも遠い。 結論: **TaroZ DD-carrier guarded submit は negative、次は mtv700/private-floor 4 artifact 復旧か、leaderboard submit なしの local analysis**。
...
## Codex maintenance handoff (2026-06-10): refactoring / user-friendly 化

この section は 2026-06-10 時点の「次に何をやるか」の入口。PPC/GSDC の leaderboard 研究ログはこの下に大量に残っているが、直近の active track は **user-friendly validation / narrow refactoring**。目的は研究結果を変えることではなく、公開 API と direct native binding を安全にし、間違った入力を kernel 境界の前で説明可能に落とすこと。

### 目的

この wave のゴールは次の 4 点。

1. **Fail fast**: NaN/Inf、負の個数、短い buffer、shape mismatch、空配列、非 finite config を CUDA / native kernel に渡さない。
2. **User-friendly**: `segfault`、CUDA illegal memory access、NumPy broadcast error、pybind の opaque な overload error ではなく、呼び出し側が修正できる `ValueError` / `RuntimeError` にする。
3. **Wrapper と binding の二重防御**: public Python wrapper は普通のユーザー向けに丁寧に検証し、direct `_gnss_gpu_*` binding も bypass されても壊れない最低限の検証を持つ。
4. **Scientific behavior は変えない**: valid input での計算結果は変えない。Refactor は boundary validation と glue repair に閉じる。PPC/GSDC score path、candidate selection、solver heuristics には触らない。

### 直近完了した commits

| Commit | Scope | 主な改善 | Verification |
|---|---|---|---|
| `f369f43` | satellite azel binding | array shape/finite/range validation | local tests + CI |
| `3af9eea` | coordinate binding | ECEF/LLA/ENU 入力 validation | local tests + CI |
| `90f6000` | native array output | zero-stride/broadcast-like output を通常 stride に修正 | local tests + CI |
| `0ccef28` | Doppler binding | velocity / observation vector validation | local tests + CI |
| `94a7941` | RAIM binding | measurement count/shape/finite validation | local tests + CI |
| `76d837e` | Python wrappers | wrapper-side validation の表現を統一 | local tests + CI |
| `24b5a8f` | multi-GNSS + Sagnac native | native range model と binding validation | local tests + CI |
| `e245fb0` | multi-GNSS wrapper | wrapper-side input validation | local tests + CI |
| `8089045` | atmosphere | correction inputs / broadcast params validation | local tests + CI |
| `ff3c618` | acquisition | sample/config/range validation | local tests + CI |
| `fb64bbe` | interference | detector/mitigation inputs validation | local tests + CI |
| `294b3e0` | tracking | `TrackingConfig` / channels / scalar/vector process validation、native config defaults、wrapper direct-use TypeError 修正 | `tests/test_tracking.py`, `tests/test_tracking_wrapper.py`, e2e smoke + CI |

### 最新 tracking patch の要点

`294b3e0` はこの wave の現在の完成形として参照する。

- `python/gnss_gpu/tracking.py`
  - `TrackingConfig` validates: `sampling_freq > 0 finite`, `intermediate_freq finite`, `integration_time > 0 finite`, DLL/PLL bandwidth finite nonnegative, `correlator_spacing > 0 finite`
  - `ChannelState` validates: PRN `1..32`, finite phases/frequencies/CN0/integrators, `code_freq > 0`
  - `ScalarTracker.initialize()` validates PRN list, code phases, Doppler frequencies, shape/length/range/finite。短い array の silent fallback を禁止
  - `ScalarTracker.process()` validates initialized channels and finite nonempty 1D signal。public `TrackingConfig` を native `_TrackingConfig` に変換し、wrapper から直接使った時の TypeError を修正
  - `VectorTracker` validates initial ECEF `(3,)` finite、process 時の `sat_ecef` / `sat_vel` shape `(n_ch, 3)` finite
- `python/gnss_gpu/_tracking_bindings.cpp`
  - `FloatArray` / `DoubleArray` に `c_style | forcecast`
  - native `TrackingConfig()` に documented defaults を付与し、uninitialized field を排除
  - constructor / direct functions で config validation
  - `batch_correlate`: signal 1D、`n_samples >= 1`、signal length、finite signal、channel count、PRN/rates/phases finite
  - `scalar_tracking_update`: channel validation、correlation size `n_channels * 6`、finite
  - `vector_tracking_update`: `dt > 0 finite`、`n_channels <= 32`、correlation/sat/nav state sizes、finite
  - `cn0_nwpr`: `n_hist >= 2`、`T > 0 finite`、history size/finite

この patch の verification pattern を次以降も使う:

```bash
PYTHONPATH=python python3 -m pytest tests/test_tracking_wrapper.py -q
cmake --build build/codex --target _gnss_gpu_tracking -j"$(nproc)"
cp build/codex/_gnss_gpu_tracking.cpython-310-x86_64-linux-gnu.so python/gnss_gpu/
PYTHONPATH=python python3 -m pytest tests/test_tracking.py tests/test_tracking_wrapper.py -q
PYTHONPATH=python python3 -m pytest tests/test_tracking.py tests/test_tracking_wrapper.py tests/test_e2e_utils.py -q
python3 -m ruff check python/gnss_gpu/tracking.py tests/test_tracking.py tests/test_tracking_wrapper.py
git diff --check
```

### Standard implementation checklist

新しい module に入る時はこの順序で進める。

1. `git status --short --branch` と `git pull --ff-only`。pull が network / remote 側で失敗したら、失敗理由を最終報告に残す。
2. 対象 module の public wrapper、native binding、header、kernel、既存 tests を読む。
3. Valid input の expected behavior と existing tests を確認する。数値挙動を変えない。
4. Invalid input を分類する:
   - shape mismatch
   - empty input
   ...
   - inconsistent `n_*` scalar と array length
5. Python wrapper は `ValueError` を優先。direct pybind は既存 style に合わせて `std::invalid_argument` / `std::runtime_error` を使う。
6. C++ pybind では `py::array_t<T, py::array::c_style | py::array::forcecast>` を検討する。non-contiguous view の reinterpret_cast は危険なので、struct buffer 扱いをする module では特に注意する。
7. Native class default constructor が Python に exposed されている場合、uninitialized field を返さない。
8. Tests は direct binding と wrapper の両方に置く。GPU-heavy path は最小 smoke にし、CPU-side validation は import だけで落ちる軽い test にする。
9. C++ binding を変えたら対象 target を rebuild し、生成 `.so` を `python/gnss_gpu/` に copy してから tests を再実行する。
10. `ruff` は Python files のみにかける。`.cpp` に ruff をかけない。
11. `git diff --check`。必要なら `pytest` scope を広げる。
12. Commit message は短く命令形。例: `Validate ephemeris binding inputs`。
13. Push 後は GitHub Actions を head SHA で確認する。CI が走る場合は success まで待つ。

### Error message style

この wave では error message を短く具体的にする。

- 良い: `gps_times must be a non-empty 1D array`
- 良い: `sat_ecef must have shape (n_channels, 3)`
- 良い: `n_sat must be positive`
- 良い: `params_flat is too small for n_sat EphemerisParams structs`
- 避ける: `invalid input`
...
- PPC/GSDC score 改善を同じ commit に混ぜない
- leaderboard artifact / private-floor artifact 探索を同じ commit に混ぜない
- `experiments/results/` の大型生成物を増やさない
- kernel algorithm の数値式を validation commit で変更しない
- unrelated old docs cleanup を混ぜない
- user changes / dirty worktree を revert しない
...
gh api -X GET repos/rsasaki0109/gnss_gpu/actions/runs \
  -f head_sha="$SHA" -f per_page=10 \
...
```bash
gh api repos/rsasaki0109/gnss_gpu/actions/runs/<RUN_ID>/jobs \
  --jq '.jobs[] | {name:.name,status:.status,conclusion:.conclusion,completed_at:.completed_at}'
```
...
詳細 memory: [`phase28-29-rmsprefilter-breakthrough-2026-05-16`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_phase28_29_rmsprefilter_breakthrough_2026_05_16.md), [`v3-ranker-wrongfix-negative-2026-05-16`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_v3_ranker_wrongfix_negative_2026_05_16.md), [`viterbi-negative-2026-05-16`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_viterbi_negative_2026_05_16.md)。
...
- 2026-05-15 SPP G+B (PLATEAU NLOS soft + IRLS) on tokyo/run1: 4 variants 全て baseline 90.84% bit-identical = **ranker 完全吸収** (Phase 21、 [`spp-gb-smoke-null-2026-05-15`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_spp_gb_smoke_null_2026_05_15.md))
...
**memory 参照**:
- [`gici-breakthrough-2026-05-14`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_gici_breakthrough_2026_05_14.md) — gici 17-variant pool 飽和、 candidate generation 既存
- [`phase28-29-rmsprefilter-breakthrough-2026-05-16`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_phase28_29_rmsprefilter_breakthrough_2026_05_16.md) — per-run conditional config の deploy 方法
- [`oracle-gap-analysis-2026-05-15`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_oracle_gap_analysis_2026_05_15.md) — 17 pool 飽和論を覆した rms_prefilter breakthrough の type、 NLOS 統合も同じく独立 lever 期待
...
- Memory store: `~/.claude/projects/-home-user-ws-gnss-gpu/memory/project_ppc_postprocess_ceiling.md`
...
このファイルはかなり長い。Claude が次に着手するなら、まずこのブロックだけ読めばよい。詳細な実験履歴は後続の §0〜§5 と、古いログ `$HOME/.claude/projects/-home-user-ws-gnss-gpu/memory/project_ppc_postprocess_ceiling.md` を参照。
...
最新の詳細ログ → `$HOME/.claude/projects/-home-user-ws-gnss-gpu/memory/project_ppc_postprocess_ceiling.md`。
...
2. `gtsam_gnss_ws/gtsam_gnss/examples_cpp/build/ambiguity_resolution <dir> 1` で FGO 実行 (1 iteration smoke)
...
|------|----------------|-----------|
| BVH hard-gate on PF3D | PLATEAU 0/301 coverage + 30% sat reject で PDOP catastrophic (-63m) | [`oracle-gap-analysis-2026-05-15`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_oracle_gap_analysis_2026_05_15.md) |
| SPP G+B (NLOS soft + IRLS) on Phase 20 pool | ranker 完全吸収、 baseline bit-identical (90.84%) | [`spp-gb-smoke-null-2026-05-15`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_spp_gb_smoke_null_2026_05_15.md) |
...
- 詳細: [`phase33-stage-b-nlos-soft-negative-2026-05-18`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_phase33_stage_b_nlos_soft_negative_2026_05_18.md)
...
詳細: [`phase33-stage-c-v5nlos-breakthrough-2026-05-18`](../../.claude/projects/-home-user-ws-gnss-gpu/memory/project_phase33_stage_c_v5nlos_breakthrough_2026_05_18.md)
...
1. **READ FIRST**: 本 §「Codex 引継ぎ」 + memory `$HOME/.claude/projects/-home-user-ws-gnss-gpu/memory/MEMORY.md`
2. **重要**: PPC 側の現 production-best route は **Phase 71 OSM road n/r2 overlay OFFICIAL 86.205492%**。 Phase43 `85.998294%` から `+0.207198pp`、 n/r2 のみ `64.426589% -> 65.669779%`。 ただし active track は 2026-05-21 時点で **GSDC2023 Kaggle pivot**。
3. **GSDC2023 active status**: TaroZ base/DD-carrier は実装・評価・提出まで完了し、Kaggle `4.048/4.860` で negative。 mtv700/private-floor 復旧は 4 core artifact missing で blocked。 次は recover artifacts or no-submit local analysis。
4. **next angle 候補** (高 ROI 順):
   ...
   - (f) **Kaggle pivot**: `/tmp/gsdc_data/gsdc2023/sdc2023` と TaroZ `../ref/gsdc2023/taroz/dataset_2023` は使える状態。 bridge full submit は `3.993/4.821`、TaroZ DD guarded full submit は `4.048/4.860` でどちらも mtv700/private-floor `~3.686/~4.710` に届かない。 現 blocker はデータ全体ではなく、private-floor/mtv700 再構成に必要な 4 artifact: `submission_20260421_0555.csv`, `submission_20260423_1450.csv`, `basecorr_pixel4xl_base_on_submission_best_cap100_plus_pixel4xl_outlier_row_20260424.csv`, `pixel5_fgo_early_raw_late_final_trip_rows.csv`。 `experiments/import_gsdc2023_private_floor_artifacts.py` の dry run は `0/4` found。
...
6. **GPL guardrail**: gici-open は `gici_open_ws/LICENSE` と upstream GitHub で GPL-3.0。gnss_gpu repo へ vendor/submodule/link/derived implementation/wrapper/config を追加しない。出力比較だけを手元 reference として扱う。
...
37. **Phase 72 GSDC2023 Kaggle pivot / TaroZ DD-carrier / mtv700 recovery (2026-05-21)**: TaroZ `dataset_2023.zip` を外部参照 `../ref/gsdc2023/taroz/` に展開し、nested `dataset_2023/base` layout を `experiments/gsdc2023_base_correction.py` で扱えるようにした。DD support は train settings40 で mean coverage `0.654`, coverage>=0.8 が `24/40`; test rows40 で mean coverage `0.788`, coverage>=0.8 が `32/40`。 train safe gate は DD selected `4/40`, wins/ties/losses `4/36/0`, score delta sum `-4.718m`。 full test candidate `experiments/results/gsdc2023_submission_taroz_dd_guarded_full_20260521.csv` は rows `71936/71936`, missing `0`, interpolated `24`, source mix `baseline=67293`, `fgo=750`, `fgo_dd_carrier=2389`, `fgo_no_tdcp=1360`, `raw_wls=120`, `interpolated=24`。Kaggle submit は `public=4.048`, `private=4.860` で reject。前回 bridge `3.993/4.821` より悪く、mtv700/private-floor `~3.686/~4.710` に届かない。private-floor recovery は `base_0555_submission`, `current_1450_submission`, `basecorr_builder_input`, `pixel5_patch` の 4 core artifacts missing。 `experiments/import_gsdc2023_private_floor_artifacts.py` を追加し、dry run `experiments/results/gsdc2023_private_floor_artifact_import_dryrun_20260521/` は `0/4` ready。次は **Kaggle submit ではなく artifact recovery**。 recover できない場合は source-family/solver 解析だけ進め、private-floor guard を通らない candidate は submit しない。
38. **Phase 73 private-floor recovery / Kaggle no-submit A/B (2026-05-21 PM)**: 4 core artifact の追加検索を `gnss_gpu / ref / /tmp / $HOME` 4 root に拡張 (`experiments/results/gsdc2023_private_floor_artifact_import_dryrun_20260521b/summary.json`) し、いずれも 0/4 found 確定。 git history (all branches), sha256 (`b60b…`, `2ff0…`, `43f9…`, `dd52…`, `359f…`) も hit 無し、 `../ref/gsdc2023/results/test_parallel/` と `experiments/results/source_selection_lowbaseline_submission_probe_20260430/` は dir ごと消失。 leaderboard 再提出不能のため、 既存 `analyze_gsdc2023_source_ab.py` で bridge submit (3.993/4.821) と TaroZ DD-guarded submit (4.048/4.860) の row/trip/phone delta を生成。 出力 `experiments/results/gsdc2023_bridge_vs_tarozdd_ab_20260521/`、 README.md に主結論。 **fgo_dd_carrier rows の 84% (2008/2389) が p95_delta_m>=3m の 13 regression trips に集中**: pixel5 8 trips (1808 DD rows、 mean p95 4.63m、 max `ebf-y` 105.72m), pixel4 2 trips (200 DD rows、 mean p95 6.30m, `mtv-k` p95 7.89m), mi8 2 trips (DD 0、 fgo realloc), pixel6pro 1 trip (DD 0、 fgo realloc)。 source-mode 内訳: DD-only 4 trips、 DD+`fgo_no_tdcp` 5 trips、 `fgo`-only 2 trips、 `fgo_no_tdcp`-only 0、 baseline-only 1 (sjc-q/pixel5 max=49.74m on base correction interpolation stale)。 DD broad hard-rule = negative 確定。 次の具体的 lever: (a) DD per-trip whitelist (`dd_carrier_dd_pairs_mean` / `dd_carrier_accepted_anchor_epochs` で per-trip score-proxy 相関再計算)、 (b) fgo_no_tdcp 拡張 (630→1360) の gate audit、 (c) base correction interpolation seed の M変更 audit (sjc-q/pixel5)、 (d) fgo-only regression trip の chunk_selection `quality_score` 閾値 audit。
39. **Phase 74 DD per-trip whitelist 閾値導出 (2026-05-21 PM)**: 12 DD-active trip 内で DD signal vs p95_delta_m の相関を取り、 `dd_anchor_epochs` が **r=-0.587** (high anchor 程 safe) と `ssc_fgo_no_tdcp` が **r=+0.706** (no_tdcp 共存で max spike 拡大) という両端 signal を特定。 推奨 gate `(dd_anchor_epochs/n_epochs >= 0.6) AND (ssc_fgo_no_tdcp == 0)` で sweep を回した結果、 bad trip 9/9 全 block、 good trip 2/3 retain (mtv-de1 のみ no_tdcp=100 で落ちる marginal、 影響 100 DD rows / p95 1.73m)。 全体 row impact (simulated revert): rows>5m **1168→49 (-96%)**、 sum\|delta\| **19087m→1095m (-94%)**、 mean per-row **0.265m→0.015m**、 per-trip shift_proxy mean **0.944m→0.065m**。 期待 Kaggle 影響は **public 4.048→3.99 帯、 private 4.860→4.82 帯** (bridge level 復帰)。 出力 `experiments/results/gsdc2023_bridge_vs_tarozdd_ab_20260521/dd_whitelist_proposal.{csv,md}`。 ただし mi8/pixel6pro/sjc-q の non-DD regression (~140 rows>5m) は別 audit が必要。 production 反映は no-submit policy 維持中: 実装は `experiments/gsdc2023_chunk_selection.py` / `gsdc2023_solver_selection.py` の DD gated branch に挿入予定だが、 private-floor reference 復旧なしでは Kaggle 検証不能のため pending。
40. **Phase 75 fgo_no_tdcp 拡張 audit + combined gate (2026-05-21 PM)**: bridge → TaroZ で `fgo_no_tdcp` rows が 630 → 1360 (+730) 拡大、 構造は **policy 撤廃**: bridge `bridge_gated_rescue_pixel4` は pixel4xl mtv-e 1 trip のみ (630 rows)、 TaroZ は phone restriction を撤廃し 8 trip で 30-300 rows 追加 (+930)、 1 trip (sjc-q/pixel5) で -200 (base へ revert、 ただし baseline trajectory 変化で max 49.74m 残る)。 追加 8 trip 中 **5/8 が regression** (rows added: bad=630/safe=300)。 chunk-level promoted no_tdcp candidate (15 chunks across 9 trips) の quality_score/mse_pr は overlap、 baseline_gap_max_m と accel_p95_m のみ marginal、 `bgmax<=15` で 8/12 bad chunks 落ちるが 33% 抑制止まり。 chunk gate 単独は不十分。 **trip-level gate `(ssc_fgo_dd_carrier==0) AND (anchor_cov>=0.6)` で 4/5 bad ntdc trip 抑制** (ebf-ww/mi8 30 rows のみ漏れる)、 safe 2/3 retain。 Combined DD + no_tdcp gate 適用結果: 25 trips pass-through, 10 reverted, 5 kept (lax-m/ebf-xx DD ok, ebf-z/lax-i ntdc ok, ebf-ww/mi8 ntdc bad-30-rows residual)。 row 影響: rows>5m **1168→290 (-75%)**、 avg per-trip shift_proxy **0.944→0.358m (-62%)**。 残 290 rows>5m は **fgo realloc** (mi8 ebf-x 115 rows、 pixel6pro mtv-ie2 29 rows) と **base correction interpolation swap** (sjc-q 48 rows、 max=49.74m) で、 これらは fgo phone-family gate + base correction seed audit (task c/d) で別途扱う。 出力 `experiments/results/gsdc2023_bridge_vs_tarozdd_ab_20260521/{no_tdcp_audit.md, per_trip_source_count_diff.csv, no_tdcp_promoted_chunks.csv, combined_gate_disposition.csv}`。
41. **Phase 76 sjc-q/pixel5 spike audit + modular framework (2026-05-21 PM)**: Phase 73-75 のオンライン分析を 5 module + 1 CLI + 6 test (50 件 pass) に整理 (`experiments/gsdc2023_ab_{source_mix,dd_signals,ntdc_chunks,gates,revert}.py` + `analyze_gsdc2023_kaggle_ab.py`)。 依存方向は左→右の一方向、 各 module は 1 責務 (loader / signal extract / chunk filter / gate / revert sim / orchestration)、 IO は loader と CLI に閉じ、 test は synthetic fixture のみで real-data 不要。 CLI を実 data で実行し Phase 75 inline output と数値一致 (rows>5m=290, sum=6385m, kept/reverted/passthrough=5/10/25, per_trip_shift_proxy=0.944→0.358m)。 続いて sjc-q/pixel5 audit を実施し Phase 75 推察 (base correction interpolation seed change) は **誤り**、 真因は **chunk_epochs config (200→100) の変更** と確定。 bridge は merged 600..800 chunk (200 epochs) で `ntdc_mse=8.21` が gate を通り `fgo_no_tdcp` 200 rows を pick、 TaroZ は同 region を 600..700+700..800 に split、 それぞれ `ntdc_mse=10.07/10.43` で gate に落ち全 baseline 化。 47/48 spike rows>5m が bridge merged chunk 内に収まり、 max=49.74m は row 735 (TaroZ chunk 700..800)。 base_correction_applied/count, observation_mask_count, tdcp_geometry_correction_count は全 bit-identical で base correction 側に変更なし。 `*_mse_pr` の値差 (bridge 17.78→TaroZ 21.18 baseline) は chunk-size 依存の集計差。 推奨 follow-up: (i) sjc-q だけ chunk_epochs=200 に戻す per-trip override、 (ii) 隣接 chunk が同 auto_source で merged MSE が gate を満たす場合の sub-chunk merge step。 期待 leverage: sjc-q 1 trip = 1/40 weight、 全 48 rows>5m と max 49.74m spike を解消すれば trip-score proxy 約 -0.5pp 回復見込み。 出力 `experiments/results/gsdc2023_bridge_vs_tarozdd_ab_20260521/sjc_q_base_correction_audit.md`。
42. **Phase 77 fgo phone-family audit → chunk_epochs が全 regression の真の root cause (2026-05-21 PM)**: fgo realloc は 5 trips のみ (ebf-x/mi8 +300, mtv-ie2/pixel6pro -100, ebf-ww/mi8 -180, mtv-k/pixel4 -200, pao-j/pixel6pro -92)。 各 trip の chunk_selection_records を bridge/TaroZ で比較したところ、 **全 5 trip とも chunk_epochs config (200→100) の変更が原因**。 ebf-x/mi8 は bridge merged 800..1000 で `fgo_mse=5.41 vs base_mse=5.31` で fgo gate fail → baseline、 TaroZ split 800..900 で `fgo_mse=6.37 vs base_mse=9.59` → fgo gate pass で **wrong-fix 100 rows 採用**、 同 trip 全体で 115 rows>5m regression。 これは sjc-q (bridge gate pass → TaroZ gate fail = 失われる fgo_no_tdcp) と **逆方向の現象** で、 両方とも chunk_epochs 変更で同じ underlying epoch quality の判定が反転している。 **全 40 trips 横断 quantification**: 21/40 trips (52.5%) で chunk-level gated_source が differ、 5091/72026 rows (7.07%) が source label flip、 **src_diff_rows ↔ rows_gt_5m の correlation r=0.834**、 21 trips のうち 13 (62%) が p95>=3m regression。 つまり **chunk_epochs 200→100 が Kaggle regression 全体の root cause** で、 Phase 73-75 の DD gate / no_tdcp gate は **症状を緩和する safety net に過ぎず、 真の修正は chunk_epochs revert**。 推奨 fix 順位: (1) chunk_epochs=200 に revert (最小変更、 5091 src_diff rows 大幅縮小見込み)、 (2) 隣接 chunks の auto_source が同で merged MSE が gate を満たせば merge re-eval (一般化解)、 (3) sjc-q 等 per-trip override map、 (4) DD-carrier path 専用に chunk_epochs を分岐。 chunk_epochs revert + 既存 DD/ntdc gate 併用で rows>5m **1168→~100 (-91%)** が見積もれ、 Kaggle public ~3.99 帯 (bridge level 完全復帰) 期待。 出力 `experiments/results/gsdc2023_bridge_vs_tarozdd_ab_20260521/{chunk_epochs_audit.md, chunk_granularity_per_trip.csv}`。
43. **Phase 78 chunk_epochs hypothesis 検証 (analytical merge + production gate replay) (2026-05-21 PM)**: TaroZ の 100-epoch sub-chunks を analytical pairwise merge して 200-epoch chunk records を再構築 (`experiments/gsdc2023_ab_chunk_merge.py`、 18 unit tests pass)、 既存 production `select_gated_chunk_source` を直接呼び出して merged gated_source 予測 → bridge actual と per-row 比較。 modular framework に統合済 (legacy code を duplicate せず import で再利用)。 **全 40 trips、 426 merged chunks、 72026 epoch rows での比較結果: 95.79% match (68995/72026)**。 Mismatch 3031 rows の内訳: (a) **dd_or_raw_enable_diff** 12 chunks / 2039 rows (bridge は `dd_carrier_fgo_enabled=False` で DD 候補なし、 TaroZ merged は DD を gate に通せる、 chunk_epochs と独立した DD-enable 設定差)、 (b) **aggregation_approximation_noise** 8 chunks / 932 rows (sub-chunk p95 を upper bound でとっているため真の merged p95 を過大評価、 production gate fail と予測するが production では pass する近似誤差)、 (c) **other** 1 chunk / 60 rows。 DD-enable 差を除外すると **chunk_epochs alone で 98.6% (71034/72026) 一致**、 残 1.4% は p95 近似誤差。 sjc-q では 6/7 merged chunks 完全一致、 唯一 mismatch chunk 600..800 が `aggregation_approximation_noise` クラスで bridge は実際に 200-epoch p95 で gate pass、 私の upper bound では fail → これが **「bridge を chunk_epochs=200 で再生成すれば fgo_no_tdcp 200 rows が回復する」ことの間接証拠**。 結論: Phase 77 hypothesis は **強力に支持確認**された、 chunk_epochs 200 revert + DD per-trip whitelist (Phase 74) の組み合わせで Kaggle public **~3.99 帯 (bridge level 完全復帰)** + 安全な DD 採用が見積もれる。 出力 `experiments/results/gsdc2023_bridge_vs_tarozdd_ab_20260521/{chunk_merge_verification.md, chunk_merge_verification_per_chunk.csv, chunk_merge_verification_per_trip.csv}`。
44. **Phase 79 CLI 統合 (`--chunk-merge-verify`) (2026-05-21 PM)**: `experiments/analyze_gsdc2023_kaggle_ab.py` に `run_chunk_merge_verification()` pure function と `--chunk-merge-verify` flag を追加、 既存 CLI 動作は不変 (default off で chunk-merge 出力は出ない、 既存 test 通過)。 wire 結果: 1 コマンドで Phase 73-78 outputs を再生成可能、 summary.json に `chunk_merge_verification` block (`total_rows`, `match_rows`, `match_pct`, `mismatch_classes`, `trips_compared`) が embed される。 mismatch 分類は modular framework の `_classify_chunk_merge_mismatch()` で `dd_or_raw_enable_diff / aggregation_approximation_noise / other` に振り分け。 新規 integration test 2 件 (`test_run_kaggle_ab_audit_chunk_merge_verify_produces_outputs` + `test_run_kaggle_ab_audit_skips_chunk_merge_outputs_by_default`) で flag on/off の両 path 検証。 実 data 再現テストで Phase 78 数値完全一致 (match_pct=0.9579, dd_or_raw_enable_diff=12chunks/2039rows, aggregation_approximation_noise=8chunks/932rows, other=1chunk/60rows, trips_compared=40)。 **modular framework 累積: 6 module + 1 CLI + 7 test ファイル / 71 tests pass / 0 regression**。 ファイル一覧: `experiments/gsdc2023_ab_{source_mix, dd_signals, ntdc_chunks, gates, revert, chunk_merge}.py` + `experiments/analyze_gsdc2023_kaggle_ab.py` + `tests/test_gsdc2023_ab_*.py` (6) + `tests/test_analyze_gsdc2023_kaggle_ab.py`。 CLI 使用例: `python3 experiments/analyze_gsdc2023_kaggle_ab.py --taroz-summary X --bridge-metrics-root Y --row-delta-csv Z --output-dir OUT --chunk-merge-verify`。
45. **Phase 80 production wire-up + ebf-x/mi8 empirical verification + 40-trip re-run (2026-05-21〜22)**: Phase 74 で導出した DD anchor coverage gate を production data flow に wire-up し、 Phase 78 hypothesis を analytical (98.6% match) と empirical (ebf-x/mi8 single-trip) の 2 軸で実 dataflow 確証、 最後に 40-trip 全 trip 再生成を kick。
    - **Wire-up (commit `744294e` Wire Phase 74 DD-carrier anchor gate into raw_bridge)**: `gsdc2023_bridge_config.py` に `dd_carrier_min_anchor_coverage` field + finite/range validation、 `gsdc2023_chunk_selection.py` に `compute_dd_carrier_anchor_coverage_ratio` pure helper、 `gsdc2023_raw_bridge.py` に `_dd_carrier_anchor_coverage` thin adapter で `_select_gated_solution` 経路に thread。 +514/-6 LOC、 6 files、 79 tests pass。 これまで helper だけ landing (commit `4225c41`) で orchestration 未接続だった inert gate が end-to-end 機能化。
    - **`build_gsdc2023_bridge_submission.py` CLI commit (commit `69acb23`)**: untracked WIP だった 492-line CLI script を main へ landing、 `--dd-carrier-min-anchor-coverage` flag を wire-up と対称的に expose。 sample_submission.csv driven 40-trip submission 生成のエントリーポイント。
    - **Empirical verification on ebf-x/mi8 (2026-05-21 PM)**: `--chunk-epochs 200` vs `--chunk-epochs 100` を実 dataflow で single-trip 走らせ、 B 方向 (Bridge merged で fgo gate fail → TaroZ split で局所 gate pass = wrong-fix) を確証。 結果: chunk_epochs=200 で sources={'baseline': 1924, 'fgo': 0}、 chunk_epochs=100 で sources={'baseline': 1624, 'fgo': 300}、 **300 rows が baseline→fgo に flip**、 position delta median **4.43m** / p95 **8.07m** / max **10.45m**。 Phase 77 prediction "ebf-x +300 fgo / +115 rows>5m" と magnitude・方向・数値で完全一致。 既存 baseline `experiments/results/gsdc2023_bridge_gated_rescue_pixel4_full_20260520/2022-04-25-21-04-us-ca-ebf-x/mi8/` (chunk_epochs=200 production) の sources={'baseline': 1924} と smoke chunk_epochs=200 出力が bit-identical で reference 等価性確認。 sjc-q/pixel5 (前回 inconclusive) と異なり ebf-x は両 config で differential が出るため hypothesis 反証ではなく per-trip dependent な現象。 runtime 374.9s (200) vs 113.4s (100) の差は sub-chunk FGO の early-fail short-circuit と整合。 出力 `/tmp/smoke_ebfx/sub_chunk{100,200}.csv` + `bridge_{100,200}/`。 memory entry `project_gsdc2023_chunk_epochs_root_cause_2026_05_21.md` に Phase 78 empirical verification 追記済。
    - **40-trip chunk_epochs=100 full re-run + row-level diff 完了 (2026-05-22 AM)**: sample_submission.csv 全 40 trip を `--chunk-epochs 100` で再生成 (`experiments/results/gsdc2023_bridge_chunk100_repro_20260522/`、 PID 254464、 jobs=1、 実 runtime ~55 min = 想定 3-4h より大幅短縮)、 既存 baseline `gsdc2023_bridge_gated_rescue_pixel4_full_20260520/` (chunk_epochs=200) との bridge_positions.csv 差分を `/tmp/diff_chunk200_vs_chunk100.py` で全 trip 取得。 **Aggregate**: src_diff_rows **2702/72026 = 3.75%**、 rows>5m **484/72026 = 0.67%**、 16/40 trips (40%) で chunk-level flip。 **Per-trip key matches**: sjc-q/pixel5 max **49.74m** (memory predicted "max 49.74m spike" と bit-exact 一致)、 ebf-x/mi8 300 rows / gt5m=115 / max=10.45m (memory predicted "+300 fgo / +115 rows>5m" 完全一致)、 mtv-ie2/pixel6pro 300 rows (memory predicted "Region 境界 0..200 → 200..300")。 **Global flip table** (prod_source → repro_source): baseline→fgo_no_tdcp 1000 (dominant A 方向)、 fgo→baseline 742、 baseline→fgo 500、 fgo_no_tdcp→baseline 300、 raw_wls→baseline 100、 baseline→raw_wls 30、 fgo→fgo_no_tdcp 30。 **Phase 77 prediction (5091 rows / 7.07%, rows>5m 1168) との数値乖離**: aggregate は予測の **53% (rows) / 41% (rows>5m)**、 21 → 16 trips。 **新解釈**: Phase 77 は TaroZ DD-guarded submission vs bridge baseline 比較で TaroZ 側に chunk_epochs=100 + DD-carrier path + ntdc_mse override が全て載っていた一方、 Phase 80 row diff は chunk_epochs config 単独効果のみを分離 = **chunk_epochs 単独で全体 regression の ~50% を説明、 残 ~50% は DD-carrier path + no_tdcp gate 差分**。 chunk_epochs revert だけでは Kaggle ~3.99 完全復帰せず、 Phase 74 DD anchor coverage gate と組み合わせ必須。 per-trip prediction の bit-exact match (sjc-q 49.74m / ebf-x +300) が hypothesis を強力に裏打ち、 aggregate 縮小は DD-carrier path も major contributor であることを示す decisive evidence。 出力 `/tmp/phase80_diff_full.log` + memory `project_gsdc2023_chunk_epochs_root_cause_2026_05_21.md` に Phase 80 セクション追記済。
    - **次の lever (revert PR scope draft)**: hypothesis 確証後の最小変更は `experiments/build_gsdc2023_bridge_submission.py` の `--chunk-epochs` default を 200 のまま固定 + production submission script の `chunk_epochs=100` 上書きを 200 に revert。 ただし Phase 80 で chunk_epochs 単独効果は全体 regression の ~50% のみなので **chunk_epochs revert + Phase 74 DD anchor coverage gate enabled** の 2 段構えが decisive、 PR scope は (1) chunk_epochs revert + (2) DD gate production default 有効化 + (3) regression test + (4) memory 反映、 大きさ ~200 LOC 想定。 private-floor reference 復旧なしでは Kaggle 検証不能のため local row-delta validation で止め、 submit は freeze 維持。
